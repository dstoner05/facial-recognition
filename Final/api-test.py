...
def db():
    ...
    def select_all(conn):
        global names, employee_ids, tokens, company_ids, Response, face_encodings, user_types
        ...
        tokens = []
        for row in rows:
            if int(token) <= int(row[0]) <= (int(token) + 10):
                tokens.append(row[0])
                company_ids.append(row[3])
                names.append(row[1])
                employee_ids.append(row[2])
                face_encodings.append(row[4])
                user_types.append(row[5])
            else:
                pass

    def create_dicts():
        global names, employee_ids, tokens, company_ids, Response, face_encodings, user_types
        global responsecode, output, Response
        Response = ''
        output = []
        for item in zip(tokens, names, employee_ids, company_ids, face_encodings, user_types):
            output.append({'ID': item[0], 'name': item[1], 'emp_id':item[2], 'comp_id':item[3],
                           'face':(item[4]),
                           'user_type':item[5]})
        responsecode = 1
        Response += "Registered employees list sent"
        # return jsonify({'response':Response, 'emp-list': output, 'responsecode':responsecode})
...
def register_new():
    ...
    new_user1 = (name, emp_id, company_id, photo, user_type)
    conn = create_connection(database)
...
def function():
    ...
    # print(unknown_photo)
        ...
        err -= 6
        return jsonify({'response':Response, 'error':err, 'empid': 0, 'empname': 0})
    ...
    def find_faces(photo):
        ...
        if face_distances[best_match_index] < .47:
            if matches[best_match_index]:
                ...
                Response += 'Unknown User Found!'
                err -= 6
                # return jsonify({'response': Response, 'empid': final_empid})
        else:
...
if __name__ == "__main__":
    app.run(host='192.168.8.13', port='5000', debug=True, threaded=True)
